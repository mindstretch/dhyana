...
const STATE_MAP: Record<string, { label: string; color: string; line: string }> = {
  restful:   { label: 'Restful Alertness', color: '#E8A855', line: 'You showed up clear. That\'s rarer than it sounds.' },
  regulated: { label: 'Regulated',         color: '#6FA89A', line: 'You showed up steady. Build from here.' },
  activated: { label: 'Activated',         color: '#C96B5A', line: 'You showed up running hot. Dhyana would know what to do with that.' },
  depleted:  { label: 'Depleted',          color: '#5A6E8A', line: 'You showed up on empty. That took something.' },
  dysreg:    { label: 'Dysregulated',      color: '#7A6880', line: 'You showed up scattered. The fact that you checked in at all means something.' },
};

const DEFAULT_STATE = { label: null, color: '#E8A855', line: 'You showed up. That\'s where it starts.' };

function buildEmail(stateKey: string | null | undefined): { subject: string; html: string } {
